...
// ★作成したfirebase.tsから関数をインポート
// ※もしエラーが出る場合は、同じフォルダにfirebase.tsがあるか確認してください
...
export default function App() {
  ...
  // ★通知用: トークンを保持するState（必要に応じて画面表示するため）
  ...
  // ★通知用: トークン取得ボタンを押した時の処理
  const handleNotificationSetup = async () => {
    const token = await requestForToken();
    if (token) {
      setFcmToken(token);
      alert("通知設定が完了しました！\nこのトークン宛に通知が送られます:\n" + token.slice(0, 20) + "...");
      // ※将来的にはここで setStudents を使い、DB(生徒データ)にトークンを保存します
    } else {
      ...
      {!loggedIn ? (
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-10">
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            {/* Left Column (Profile & Student List) */}
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="pt-8 border-t border-white/20">
                      ...
                      <button 
                        ...
                        className="flex items-center justify-center gap-2 w-full py-4 mb-3 bg-purple-600 text-white rounded-2xl font-black text-xs cursor-pointer hover:bg-purple-500 shadow-lg transition-all"
                      >
                         🔔 通知を受け取る設定
                      </button>
                      ...
                      <div className="flex flex-col md:flex-row gap-4 items-end">
                        <div className="flex-1 w-full">
                          ...
                          <div className="flex items-center gap-2 bg-slate-700 p-2 rounded-xl border border-slate-600 focus-within:border-rose-500 transition-colors">
                            ...
                            <input 
                              type="text" 
                              value={currentStudent.meetingUrl || ""} 
                              onChange={(e) => updateMeetingUrl(e.target.value)}
                              placeholder="https://zoom.us/j/..." 
                              className="w-full bg-transparent border-none outline-none text-sm font-bold text-white placeholder-slate-500"
                            />
                          </div>
                        </div>
                        {currentStudent.meetingUrl && (
                          <a href={currentStudent.meetingUrl} target="_blank" rel="noreferrer" className="w-full md:w-auto px-8 py-4 bg-rose-500 hover:bg-rose-400 text-white rounded-xl font-black text-xs whitespace-nowrap shadow-lg transition-all text-center">
                            入室する 🚀
                          </a>
                        )}
                      </div>
                            ...
                            <div className="grid grid-cols-4 gap-4">
                              {Object.keys(currentStudent.subjects).map(subj => (
                                <div key={subj}>
                                  ...
                                  <input 
                                    type="number" 
                                    disabled={t.status === "approved" || (role === "teacher" && t.status === "draft")}
                                    value={t.scores[subj]?.result || 0} 
                                    onChange={(e) => updateTestScore(i, subj, Number(e.target.value))}
                                    className="w-full p-2 rounded-lg bg-white/50 border border-slate-200 text-center font-bold disabled:opacity-50"
                                  />
                                </div>
                              ))}
                            </div>
